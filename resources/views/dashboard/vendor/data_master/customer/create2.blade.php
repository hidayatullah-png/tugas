...
@section('content')
    <div class="page-header">
        <h3 class="page-title">
            ...
            </span> Tambah Customer (Metode File Path)
        </h3>
        <nav aria-label="breadcrumb">
            <ul class="breadcrumb">
                <li class="breadcrumb-item active" aria-current="page">
                    <span></span>Smile for the camera :D <i class="mdi mdi-camera icon-sm text-primary align-middle"></i>
                </li>
            </ul>
        </nav>
    </div>
        ...
        <div class="col-md-7 grid-margin stretch-card">
            <div class="card shadow-sm border-0">
                <div class="card-body">
                    ...
                    {{-- ROUTE DIUBAH KE STORE2 --}}
                    <form action="{{ route('vendor.customer.store2') }}" method="POST" id="formCustomer">
                        @csrf
                        <div class="form-group">
                            <label class="font-weight-bold">Nama Lengkap</label>
                            <input type="text" name="nama_customer" class="form-control"
                                placeholder="Contoh: Hidayatullah Sukma Dewi" required>
                        </div>
                        ...
                        <button type="submit" id="btnSimpan" class="btn btn-gradient-primary w-100 shadow" disabled>
                            <i class="mdi mdi-content-save"></i> Simpan Data & Foto (Path)
                        </button>
                    </form>
                </div>
            </div>
...
@section('scripts')
    ...
    <script>
        ...
        const fetchData = (id, targetId, routeName) => {
            ...
            axios.get(routeName, { params: { id: id } })
                .then(res => {
        ...
        document.getElementById('formCustomer').addEventListener('submit', function () {
            Swal.fire({
                ...
                text: 'Foto sedang diproses dan disimpan ke folder storage',
